...
def build_model(model_name, num_classes=10, freeze=1, name="model"):
    assert model_name.lower() in model_dict.keys(), f"Please, check pretrained model list {list(model_dict.keys())}"

    last_activation = "softmax" if num_classes > 1 else "sigmoid"

    
    base_model = model_dict[model_name.lower()](include_top=False, weights="imagenet", pooling="avg")

    if freeze:
        base_model.trainable = False
    
    output = layers.Dropout(0.5, name=f"{name}_dropout")(base_model.output)
